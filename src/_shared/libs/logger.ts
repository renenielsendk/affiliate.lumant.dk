 ...
 */
export const initializeAPILogger = (req: Request) => {
  ...
  createChildLogger({ trace: 'initializeAPILogger' });
};
...
/**
 * Initialize Logger.
 * @param params - The extra params to be associated with the logger.
 */
export const initializeLogger = ({ name }: { name: string; [key: string]: any }) => {
  // Generating a unique id for each request
  const id = customAlphabet('1234567890abcdef', 20)();
